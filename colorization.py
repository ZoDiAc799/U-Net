...
class MyDilatedConv2d(MyConv2d):
    ...
    def forward(self, input):
        ...
        conv = F.conv2d(input,self.weight,self.bias,padding=self.padding,dilation=2)
        usample = torch.nn.Upsample(scale_factor=(8/6,8/6),mode='bicubic')
        return usample(conv)
...
if __name__ == '__main__':
    ...
    colors = np.load(args.colors, allow_pickle=True,encoding='latin1')[0]
    num_colors = np.shape(colors)[0]
